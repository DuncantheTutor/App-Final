...
- **Client:** React Native (Expo), native Android project under `android/`. Display name **Erdos**; install id **`com.duncanharper42.appv2build2`** until the `com.erdos.app` Android app exists on Firebase project `nfc-app-7095e`. Kotlin `namespace` stays `com.erdos`. Orchestration is moving out of `app/MainApp.tsx` into feature modules: `app/shell/` (routing), `app/messaging/useMessagingController.ts` (chat/message state), `app/session/useBackendSession.ts` (device session refs), `app/session/useSignedInSession.ts` (signed-in / splash / encrypted-sync flags), `app/feed/useFeedController.ts` (post rows, mute/seen prefs, list paging), `app/feed/useFeedSync.ts` (encrypted-post pull, poll-on-open, older-page load, home-feed snapshot), `app/feed/useFeedReactionListeners.ts` (live feed reaction pills), `app/friends/useFriendsController.ts` (roster / unfriend / link-graph), `app/profile/useProfileController.ts` (my-profile fields + friend profile-card cache), `app/profile/useEncryptedProfileSync.ts` (encrypted-profile snapshots), `app/posts/` (new-post draft, media pick, encrypt upload), `app/notifications/useNotificationPermissionGate.ts` (pre-prompt + OS permission snapshot), `app/addFriend/` (pair-offer register + dual-confirm parent callables + session wait), `app/chat/useStartChatComposer.ts` (Start Chat / broadcast-picker drafts). Dual-confirm pairing still composes through `MainApp`. Submit and dismiss controls (**Send**, **Publish**, **Post**, **Cancel**, **Back**, **Close**, **Not now**) acknowledge press with an in-bounds flash or paper-plane nudge (no button scale) plus a light haptic; top nav screen icons tick. Settings → **Haptic feedback** can disable this; the switch still works when Android system touch haptics are off. The previous tree **`C:\Users\dunca\OneDrive\Desktop\App FInal V3`** is a read-only archive.
...
- **Sign-up / sign-in** with username, email, and password. **6-digit email OTP is temporarily disabled** (`EMAIL_OTP_ENABLED = false` in `app/theme/preludeConstants.ts`); OTP screens and `requestEmailOtp` / `verifyEmailOtp` remain in the tree. Product rule R21 (OTP after sign-out) still stands when the flag is flipped back.
...
- **New-post composer (`publishPost` screen):** media tray uses **icon-only** take-photo / add-photos / add-video / clear actions; tapping the media slot offers **Take photo** or **Choose from gallery**. Cancel and Publish live in a **sticky bottom footer** that adds `Math.max(insets.bottom, 16)` Android padding so the primary action never sits inside the gesture/nav reserved area. The Android navigation bar tint follows `theme.background` (set globally by the screen-level nav-bar effect in `MainApp.tsx`).
