...
- Client split (in progress): `app/shell/` (routing), `app/messaging/useMessagingController.ts` (chat state), `app/session/useBackendSession.ts` (device session), `app/feed/useFeedController.ts` (post rows), `app/addFriend/` (pair-offer register)
