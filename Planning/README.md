...
Product specs, MVP plans, parity checklists, chat exports, and execution notes for **App Final V3**.

## Where to work

- **Code and builds:** repository root `C:\Users\dunca\OneDrive\Desktop\App FInal V3` (open that folder in Cursor).
...
- Client split (in progress): `app/shell/` (routing), `app/messaging/useMessagingController.ts` (chat state), `app/session/useBackendSession.ts` (device session), `app/feed/useFeedController.ts` (post rows)
...
- `REAL_OTP_SIGNIN_COMMANDS.md` — PowerShell commands for Firebase/APK from App Final V3
