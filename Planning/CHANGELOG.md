...
- **Client module split (start):** Routing (`view` / `homeTab`) lives in `app/shell/`; `MainApp` chat/message writes go through `useMessagingController` (`applyChats` / `applyMessages` / `replaceInbox`). Backend session refs live in `app/session/useBackendSession`; signed-in / splash / encrypted-sync flags live in `app/session/useSignedInSession`; feed post rows, mute/seen prefs, and list paging flags live in `app/feed/useFeedController`; encrypted-post pull, poll-on-open, older-page load, and home-feed snapshot live in `app/feed/useFeedSync`; live feed reaction pills live in `app/feed/useFeedReactionListeners`; fullscreen post comment drafts live in `app/feed/useFullscreenPostThread`; emoji picker target lives in `app/feed/useReactionPicker`; friend roster / unfriend / link-graph state lives in `app/friends/useFriendsController`; my-profile fields and friend profile-card cache live in `app/profile/useProfileController`; encrypted-profile snapshots live in `app/profile/useEncryptedProfileSync`; new-post draft + camera/gallery pick + encrypt upload live in `app/posts/`; photo-editor + crop overlay session live in `app/media/usePhotoEditorSession`; notification pre-prompt + OS permission snapshot live in `app/notifications/useNotificationPermissionGate`; pair-offer register and dual-confirm parent callables live in `app/addFriend/`; Start Chat / broadcast-picker drafts live in `app/chat/useStartChatComposer`.
...
- **Feed media:** Home feed photos and videos meet the left and right screen edges.
- **Email OTP temporarily off:** Login and signup use email/password only (`EMAIL_OTP_ENABLED = false`). OTP screens and callables remain in the tree; flip the flag to restore 6-digit verification.
- **Login/signup password eye:** Show/hide control sits inside the right of the password field so email and password inputs are the same width.
- **New-post photo editor:** After editing a photo for a post, **Done** adds it to the composer — no extra **Post** preview step.
- **Main nav swipe:** Swipe left/right between profile, friends, chats, feed, add friend, and settings; the next screen follows the finger. Feed accepts the swipe anywhere except on a multi-image photo carousel.
...
### Added (Sep 2026)
- **Haptic settings:** In-app toggle persisted with appearance prefs. OS haptic-off is detected on Android for a hint, but the switch still enables Erdos vibration.
...
- **Boot roster leftovers:** After a non-empty `listMyFriends` result, extra local-only friends are dropped so they cannot re-enter the publish envelope.
- **Post photo editor OK vs Post:** Crop overlay confirm is **Done**; preview keyboard accessory uses **Post** / **Send** / **Use photo** and submits.
- **Publish blocked by leftover friends:** New posts encrypt for self + accepted `u_*` friends only. `createEncryptedPost` and `updateEncryptedPost` skip stale envelope uids instead of failing with **You can only view profiles of friends.**
- **Chat media border:** Even 2px inset on top and sides hugging rounded corners; bottom inset when there is no caption (pending preview matches).
- **Press-ack send clip:** Paper-plane Send no longer clips its 5px nudge (`overflow: hidden` only on flash buttons).
- **Notification gate after logout:** OS-permission refresh / Allow no longer applies after sign-out.
- **Auth sign-out memory:** Firebase `null` auth (not only the Logout button) now clears feed mute/seen, post/message watermarks, recipient key cache, and accepted-friend UID sets so the next account cannot inherit them.
