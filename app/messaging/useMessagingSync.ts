...
import {
  ...
  attachConversationMessageListener,
...
export type UseMessagingSyncOptions = {
  ...
  /** When on home, only attach the inbox listener on the Chats tab (not Feed). */

  homeTab?: string;

  /** Local chat id while the chat screen is open — triggers a per-thread listener. */
...
export function useMessagingSync(options: UseMessagingSyncOptions) {
  ...
  /** Open chat: Firestore per-thread listener only (no periodic callable polling). */

  useEffect(() => {

    if (viewScreen !== "chat" || !signedIn || demoOfflineMode || !initialServerSyncDone) {
    ...
    if (!session || !activeChatLocalId) return;



    const conversationId = resolveConversationId(activeChatLocalId);

    return attachConversationMessageListener({
   ...
   * Inbox hint listener: home → Chats tab only. Off on Feed and while a chat is

   * open (per-thread listener handles that screen).
  ...
  useEffect(() => {
    ...
    if (viewScreen === "chat" && activeChatLocalId) return;

    const onChatsSurface = viewScreen === "home" && homeTab === "chats";

    if (!onChatsSurface) return;
