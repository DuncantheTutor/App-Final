...
- **Post photo editor (Sep 17 testing):** Adding a photo to a post — crop confirm is **Done** (not **OK**). Preview footer and iOS caption accessory say **Post** (not **OK**) and that accessory submits. A caption typed in the editor must appear in the publish composer and on the feed post.
- **Publish own posts:** Publishing must not alert **Post not published / You can only view profiles of friends.** The new post stays on your feed. Recipients are self + accepted `u_*` friends (stale local roster rows are skipped). **Functions deploy** of `createEncryptedPost` is recommended so the server also skips leftover recipient uids.
- **Chat media border:** Photo/video bubbles use the same **2px** inset on top and sides (hugging the rounded corners). Uncaptioned media also get that 2px on the **bottom**. Caption still sits flush under the image with no extra bottom inset.
...
- **Login / OTP keyboard:** Login and signup auth screens stay visually centered; focusing any field scrolls/pads so inputs and action buttons stay above the keyboard (no full-screen `KeyboardAvoidingView` jump). Login OTP: **Request OTP code** + **Verify OTP** visible. Signup OTP: **Request new OTP** + **Verify OTP** visible.
...
- **Push (both phones):** notification permission granted, push credentials configured (`RUN_MVP_LOCALLY_AND_ON_PHONE.md`). Background phone B → A sends a chat message → B gets a heads-up within ~1–3 s; same for A receiving from B. Requires **Functions deploy** (`registerPushToken` + push delivery logging) + new APK on both devices. After opening the app once with notifications allowed, Firestore should show `users/{uid}/pushTokens/{deviceId}-fcm` (and optionally `-expo`).
...
## Android release build (Sep 2026)

- **`npm run apk:release`:** Gradle must pass `:app:processReleaseResources`. Adaptive icons use `@color/iconBackground` (`#000000`) plus `@drawable/erdos_e`; splash uses `@drawable/ic_launcher_background`. If AAPT reports `color/iconBackground` or `mipmap/ic_launcher_foreground` missing, those names were stripped from `android/app/src/main/res`.
...
- **Notification pre-prompt (first launch):** After **sign-in** (not before account creation), while still signed in, the app shows **Stay in the loop** with **Allow notifications** / **Not now** before the home feed is usable when OS permission is **undetermined** or **denied** (Android 13+ default) and the user has not yet completed the in-app **Allow** flow. Chats and feed sync continue in the background during this screen. **Allow** → OS permission sheet → home. **Not now** → home without OS prompt; pre-prompt may show again on the **next cold start** until Allow was tapped once. Users who already **granted** at the OS level skip this screen. **Requires new release APK.**
...
- **Photo editor (chat photos):** After editing, tap **Done** — chat-sized preview above compact composer; caption (if any) sits in the **same bubble** as the image (even **2px** inset on top/sides hugging the 12px bubble / 10px inner corners; **2px bottom** as well when there is no caption; full-width text below when there is). **Send** with empty text still sends the photo. Sent photos with captions use the same unified bubble (not a separate text bubble under the image).
...
- **Photo editor & preview footer:** the editor is **non-scrollable** (fixed layout: canvas + tool dock + sticky footer). **Filters** open from a **sliders** icon (modal), not a permanent chip row. **Crop** (expo crop overlay) confirms with **Done**, not **OK**. Preview shows two large footer buttons — **Back** and **Send** (or **Post** / **Use photo**) — and the iOS caption accessory uses that same submit label. Android system nav bar background/border match the active theme (light/dark/pink/green). "Clear overlays" is the trash icon in the tool row.
...
- **New-post composer:** the media tray uses icon-only buttons (camera, gallery, video, trash) instead of text. Tapping the media slot offers **Take photo** or **Choose from gallery**. Cancel and Publish live in a sticky bottom footer with a hairline top divider that sits at `Math.max(insets.bottom, 16) + 8` so Publish is never inside the Android gesture/nav area. Scrolling the body never hides the Publish button.
...
- Cold start while **still signed in** (did not Logout): the same splash shows until Firebase initial auth resolves, local cache is hydrated, and for at least ~500ms from launch—login must not flash first. A fallback restore runs if Firebase has a persisted user but the first auth callback did not hydrate the app. **`claimDeviceSession`** and related session init continue **after** the splash; cached feed/chats may appear before the server session is ready. After claim succeeds, send / push / friend-profile loads that wait on the session can proceed immediately (they do not wait for a later React re-render). Opening a friend profile during that claim window waits a few seconds before showing “no internet.”
...
- **New post:** Tap the **create** (pencil) icon (leftmost in the top nav bar) from feed or any screen that shows `HomeTopNavBar`. Opens the **publish** composer with the **same top nav**; the **create** icon is **highlighted** while composing. Screen **scrolls**; footer **Cancel** / **Publish** use safe-area padding. Top **media slot** (tap for **Take photo** or **Choose from gallery**); caption below; **Camera / Gallery / Video / Clear** icon row. Camera captures go through the same photo **editor** as gallery picks; **Continue** and **Post** sit above the Android gesture/nav inset (`SafeAreaView` + `photoEditorFooterPadding`). **Settings** is second-from-right; **logout** is rightmost. There is **no** bottom feed **New post** bar.
