...
- **Haptic press ack:** **Send** / **Publish** / **Post** keep the same button size; send uses a short paper-plane nudge, publish/post a flash, plus a light tick. Top nav (chats, friends, feed, profile, …) ticks on tap. Settings → **Haptic feedback** turns it off. If Android **touch haptics** are already off (or the device has no vibrator), the switch stays off and cannot be turned on. Re-open Settings after changing the OS toggle.
