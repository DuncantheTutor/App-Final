...
- Cold start while **still signed in** (did not Logout): the same splash shows until Firebase initial auth resolves, local cache is hydrated, and for at least ~500ms from launch—login must not flash first. A fallback restore runs if Firebase has a persisted user but the first auth callback did not hydrate the app. **`claimDeviceSession`** and related session init continue **after** the splash; cached feed/chats may appear before the server session is ready.
